...
class ChildNode(GameNode):
    ...
    def _accept(
        self, parent_board: chess.Board, visitor: BaseVisitor[ResultT], *, sidelines: bool = True
    ) -> None:
...
class Headers(MutableMapping[str, str]):
    def __init__(
        self, data: Optional[Union[Mapping[str, str], Iterable[Tuple[str, str]]]] = None, **kwargs: str
    ) -> None:
    ...
    def __repr__(self) -> str:
        return "{}({})".format(
            type(self).__name__, ", ".join("{}={!r}".format(key, value) for key, value in self.items())
        )
